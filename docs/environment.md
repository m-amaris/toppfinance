...
| `OPENROUTER_API_KEY` | Integraciones | No | `sk-or-...` | Clave de acceso a OpenRouter. |
| `OPENROUTER_DEFAULT_MODEL` | Integraciones | No | `openai/gpt-5-mini` | Modelo principal. |
| `OPENROUTER_FALLBACK_MODELS` | Integraciones | No | `anthropic/claude-sonnet-4.5,google/gemini-3-flash-preview` | Lista de modelos alternativos. |
| `OPENROUTER_ZDR` | Integraciones | No | `true` | Flag o política asociada al tratamiento de datos. |
| `RATE_LIMIT_MAX` | API | No | `1000` | Máximo requests por ventana. |
| `RATE_LIMIT_WINDOW_MS` | API | No | `60000` | Ventana de rate limit (ms). |
| `RATE_LIMIT_ALLOWLIST` | API | No | `10.0.0.1,10.0.0.2` | IPs exentas de rate limit. |
| `RATE_LIMIT_BAN_DURATION_MS` | API | No | `0` | Duración de ban (ms). |
| `RATE_LIMIT_CACHE_SIZE` | API | No | `5000` | Tamaño caché de rate limit. |
| `SEED_ADMIN_EMAIL` | Seed | No | `admin@example.com` | Email del admin inicial. |
| `SEED_ADMIN_NAME` | Seed | No | `Admin` | Nombre del admin inicial. |
| `SEED_ADMIN_PASSWORD` | Seed | No | `changeme` | Password del admin inicial. |
...
| `SEED_MEMBER_PASSWORD` | Seed | No | `changeme` | Password del usuario miembro inicial. |
...
- Mantener separados los valores de local, preview y producción.
...
| Variable | Valor preview |
| --- | --- |
| `NODE_ENV` | `production` |
| `DATABASE_URL` | BD aislada por PR (ej. `toppfinance_pr_123`) |
| `APP_URL` | `https://pr-123.preview.toppfinance.app` |
| `CORS_ORIGIN` | `https://pr-123.preview.toppfinance.app` |
| `SESSION_COOKIE_NAME` | `toppfinance_session_pr_123` |
| `COOKIE_SECURE` | `true` |
| `BACKUP_DIR` | `/app/backups/preview-123` |
| `OPENROUTER_API_KEY` | Shared test key o mock |

Variables inyectadas por CI/CD en preview:
- `DATABASE_URL`: Creada dinámicamente por CI
- `APP_URL`: Derivada de número de PR
- `CORS_ORIGIN`: Igual que APP_URL
...
- `SESSION_COOKIE_NAME` con prefijo `__Host-` si usas cookies de sesión seguras: `__Host-toppfinance_session`.
- `RATE_LIMIT_*` ajustados a tráfico esperado.
...
# Validar .env.production (en servidor)
npm run secrets validate .env.production

# Generar secrets seguros
npm run secrets generate password 24
npm run secrets generate secret 32
```

## Generación de .env por entorno

```bash
# Desarrollo
npm run secrets create development

# Preview (PR #123)
npm run secrets create preview

# Producción
npm run secrets create production
```