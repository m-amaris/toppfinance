...
import { ApiError, toApiError, isApiError, API_PREFIX } from './apiErrors.js'
import { success, created, paginated, getPaginationParams } from './apiResponse.js'
...
/**
 * Build the Fastify application: plugins, error handler, routes and optional
 * static serving of the web build. Does NOT call listen() — the caller
 * (server.ts in production, tests via inject) decides whether to listen.
 */
export async function buildApp(opts: { logger?: boolean; enableRateLimit?: boolean } = {}): Promise<FastifyInstance> {
...
// 4. Rate limiting (optional: tests disable it because of hardcoded per-route overrides)
if (opts.enableRateLimit ?? true) {
...
// Accounts routes - POST to create
