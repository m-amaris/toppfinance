...
import { useCsvPreview, useCsvCommit } from '../hooks/useQueries.ts'
...
function getMonthLabel(fecha: string) {
  return new Date(fecha + 'T12:00:00').toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })
}

export function Transacciones() {
  const { transacciones, categorias, isApiEnabled } = useFinanzas()
  const previewMutation = useCsvPreview()
  const commitMutation = useCsvCommit()

  const [search, setSearch] = useState('')
  const [tipo, setTipo] = useState<'todos' | 'gasto' | 'ingreso' | 'ahorro' | 'transferencia' | 'ajuste'>('todos')
  const [categoria, setCategoria] = useState('todas')
  const [desde, setDesde] = useState('')
  const [hasta, setHasta] = useState('')
  const [csvFile, setCsvFile] = useState<File | null>(null)
  const [csvPreview, setCsvPreview] = useState<{ summary: any; rows: any[]; importBatch: { id: string } } | null>(null)
  const [csvResult, setCsvResult] = useState<{ summary: any } | null>(null)
  ...
  async function handlePreview() {
    ...
    try {
      ...
      const preview = await previewMutation.mutateAsync({
        fileName: csvFile.name || 'movimientos.csv',
        content,
  ...
  async function handleCommit(includeDuplicates = false) {
    ...
    try {
      ...
      const result = await commitMutation.mutateAsync({ id: csvPreview.importBatch.id, body: { includeDuplicates, rows } })
      setCsvResult(result)
      setCsvPreview(null)
      setCsvFile(null)
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-4 pb-24">
        ...
        <div className="mx-4 card p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-[var(--color-text-muted)]">{csvPreview ? `${csvPreview.summary.total} filas revisadas` : 'Preview y confirmación'}</p>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <button type="button" disabled={previewMutation.isPending || !csvFile} onClick={handlePreview} className="btn-primary min-h-[40px] px-3 flex items-center gap-1.5">
              {previewMutation.isPending ? <Icon name="Loader2" size={14} className="animate-spin" /> : <Icon name="Eye" size={14} />}
              Preview
            </button>
          </div>
          ...
          {csvPreview && (
            <div className="flex flex-col gap-3">
                    ...
                    <div key={`${row.rowNumber}-${row.sourceHash}`} className="px-3 py-2 text-xs flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[var(--color-text-muted)] truncate">
                          {row.display.date || 'Sin fecha'} · {row.display.typeLabel} · {row.display.amount == null ? 'Importe inválido' : formatEsNumber(row.display.amount, { suffix: '€' })}
                        </p>
                        {reason && (
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" disabled={commitMutation.isPending || csvPreview.summary.ready === 0} onClick={() => handleCommit(false)} className="btn-primary min-h-[42px] flex items-center justify-center gap-1.5">
                  <Icon name="Upload" size={14} />
                  Importar válidas
                </button>
                <button type="button" disabled={commitMutation.isPending || csvPreview.rows.filter(row => row.draft).length === 0} onClick={() => handleCommit(true)} className="btn-secondary min-h-[42px] flex items-center justify-center gap-1.5">
      ...
      <div className="mx-4 card p-4 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <select aria-label="Filtrar por tipo" value={tipo} onChange={e => setTipo(e.target.value as typeof tipo)} className="input-base">
          ...
          <select aria-label="Filtrar por categoría" value={categoria} onChange={e => setCategoria(e.target.value)} className="input-base">
            ...
            {categorias.map((cat: Category) => <option key={cat.id} value={cat.id}>{cat.label}</option>)}
          </select>
        </div>
        <div className="flex items-center gap-2">
