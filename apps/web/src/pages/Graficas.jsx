...
const PieTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="card px-3 py-2 text-xs shadow-md">
      ...
      <p className="text-[var(--color-text-muted)]">{p.payload.percent}%</p>
    </div>
  )
}
...
function yearTotalsFromSeries(series) {
  const acc = new Map()
  series.forEach(item => {
...
export function Graficas() {
  const { patrimonioMensual, monthKeys, yearKeys, getMonthStats, getYearStats, presupuestos, CATEGORIAS_GASTO, configuracion } = useFinanzas()
  ...
  const presupuestosData = useMemo(() => {
    ...
    return presupuestos.map(p => {
      const cat = CATEGORIAS_GASTO.find(c => c.id === p.categoria)
