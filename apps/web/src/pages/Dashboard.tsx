...
import type { Transaction, Account } from '../types/index.ts'
...
export function Dashboard({ onNavigate }: DashboardProps) {
  const { monthKeys, getMonthStats, getPreviousMonthKey, getPatrimonioParaMes, transacciones, cuentas, configuracion, patrimonioMensual } = useFinanzas()
  const { user, isLoading: syncing } = useAuth()
    ...
    <div className="flex flex-col gap-5 pb-24" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <div className="mx-4">
        ...
        <div className="card divide-y divide-[var(--color-divider)]">
          {recientes.length ? recientes.map((tx: Transaction) => <TransactionItem key={tx.id} tx={tx} />) : <div className="px-4 py-8 text-sm text-[var(--color-text-muted)]">No hay movimientos en este mes.</div>}
        </div>
      </div>
    </div>
  )
