...
export function Configuracion() {
  const { configuracion, actualizarConfiguracion, cuentas, presupuestos, actualizarPresupuesto, actualizarCategoria, isApiEnabled } = useFinanzas()
  const { user, logout } = useAuth()

  const handleUpdateCategory = (id: string, patch: Partial<Category>) => {
    ...
    <div className="flex flex-col gap-4 pb-24">
      ...
      <div className="mx-4">
        ...
        <div className="card divide-y divide-[var(--color-divider)]">
          {CATEGORIAS_GASTO.map((cat: Category) => {
            ...
            return (
              <div key={cat.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="w-24 relative">
                  <input type="number" min="0" value={p?.limite || 0} onChange={e => actualizarPresupuesto(cat.id, Number(e.target.value))} className="input-base py-1 px-2 text-right pr-6 text-sm" />
      ...
      {isApiEnabled && (
        <div className="mx-4 card p-4 flex items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold">{user?.displayName || 'Sesión activa'}</h2>
