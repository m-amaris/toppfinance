...
export function AddTransactionModal({ onClose }) {
  const { categoriasPorTipo, agregarTransaccion, configuracion, cuentas } = useFinanzas()
  ...
  async function handleSubmit(e) {
    ...
    try {
      await agregarTransaccion({
        ...
        importe: tipo === 'ingreso' || tipo === 'ajuste' ? imp : -imp,
        categoria: form.categoria,
        fecha: form.fecha,
        tipo,
        cuentaId: form.cuentaId,
        cuentaDestinoId: tipo === 'ahorro' || tipo === 'transferencia' ? form.cuentaDestinoId : null,
      })
      onClose()
    } catch (submitError) {
        ...
        <div className="overflow-y-auto px-4 sm:px-6 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] overscroll-contain">
          <div className="grid grid-cols-2 gap-2 mb-5">
            {[
              ...
              <button
                key={t}
                type="button"
                onClick={() => { setTipo(t); setForm(p => ({ ...p, categoria: '' })) }}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="sticky bottom-0 bg-[var(--color-surface)] pt-2 pb-[max(0.25rem,env(safe-area-inset-bottom))]">
              <button type="submit" disabled={submitting} className="btn-primary w-full min-h-[48px] flex items-center justify-center gap-2">
